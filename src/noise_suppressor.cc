...
void FormExtendedFrame(rtc::ArrayView<const float, kNsFrameSize> frame,
  ...
  std::copy(
      ...
      extended_frame
          .begin()); // 将old_data数据放入到extend_frame中，old_data数据只有96点
  std::copy(
      frame.begin(), frame.end(),
      extended_frame.begin() +
          old_data.size()); // 将frame的数据放到后extend_frame的后160个点的位置
  std::copy(
      extended_frame.end() - old_data.size(), extended_frame.end(),
      old_data.begin()); // 将extended_frame的后96个点的数据重新放到old_data中
}
...
NoiseSuppressor::ChannelState::ChannelState(
    const SuppressionParams &suppression_params, size_t num_bands)
...
NoiseSuppressor::NoiseSuppressor(const NsConfig &config, size_t sample_rate_hz,
                                 size_t num_channels)
    : num_bands_(NumBandsForRate(sample_rate_hz)), num_channels_(num_channels),
